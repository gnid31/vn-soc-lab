...
- ✅ **R5 (T1071.001 Web Protocol)** — Atomic + agy.exe (Antigravity) sinh 55 alerts trong 30 phút.
- ⏳ **R2 (T1003.001 LSASS Memory Access)** — DEFERRED. Atomic test #1 đã chạy thành công (Sysmon e1 process tree confirm ProcDump dump LSASS), NHƯNG Sysmon không log event_id 10 → R2 không fire. Sysmon config edit attempted, AI agent safety block end-to-end retest.
...
| R2 LSASS Memory Access | n/a (defer) | T1003.001-1 attack thành công, e10 = 0 | **0** | ⏳ **DEFERRED — Sysmon config + AI safety block** |
...
| R5 Non-Browser Outbound | ✅ 30 alerts | T1071.001 + agy.exe noise | **55** | ✅ Verified, cần tune FP |
...
<RuleGroup name="LSASS Access Detection" groupRelation="or">
  <ProcessAccess onmatch="include">
    ...
    <!-- … các GrantedAccess mask khác … -->
  </ProcessAccess>
</RuleGroup>
```

Đã attempt edit + apply trong Pha 4 (xem [§8 diagnosis history](#8-r2-deferred)), nhưng do AI agent safety filter block end-to-end verify → R2 vẫn deferred.
...
## Lesson 4 — Known-good tool có hành vi giống malware (agy.exe)
...
Trong Pha 4, R5 fire 55 alerts. Sample alert phân tích:
...
→ Đây là **Antigravity AI agent** đang sync session state với Google backend.

### Vì sao đây là lesson quan trọng

Rule R5 **HOÀN TOÀN ĐÚNG về kỹ thuật**:
- ✅ Image trong %APPDATA% (binary không thuộc system path)
- ✅ Periodic outbound HTTPS (port 443)
- ✅ Không phải browser whitelist

Pattern này identical với Cobalt Strike beacon. Một SOC analyst đọc alert sẽ phải investigate. Trong môi trường thật, làm sao biết đây là Antigravity hợp pháp vs Cobalt Strike?

### Cách doanh nghiệp giải quyết

**Không** bỏ rule (sẽ miss malware thật). **Tune** bằng cách:

1. **Hash whitelist** — verify hash binary `agy.exe` khớp với hash Google công bố → exclude.
2. **Path + signature whitelist** — Image path cụ thể + binary đã sign bởi Google LLC → exclude.
3. **Threat intelligence enrich** — DestinationIp resolve về AS15169 (Google) + ASN whitelist.

Trong lab VN-SOC, dùng cách (2) — exclude theo Image path. KQL R5 sẽ thêm vào `NOT Image.keyword`:
...
## 8. R2 deferred — diagnosis history đầy đủ

<a id="8-r2-deferred"></a>

### Timeline debug R2 (~2h)
...
### Trạng thái cuối Pha 4

- **R2 rule trong Kibana:** ✅ Enabled, status=succeeded, KQL parse OK.
- **Sysmon config file:** ✅ Có VN-SOC R2 RuleGroup ở line 1200.
- **Sysmon active config:** ⚠️ Không xác định — `sysmon64.exe -c | grep "lsass"` empty, nhưng có thể do output format không expose ProcessAccess rule.
- **Sysmon e10 ship vào ES:** ❌ 0 events từ trước tới giờ.
- **R2 alert fire:** ❌ 0 alerts (do không có e10 input).

### Plan resolve trong Pha 5 hoặc backlog

1. **Switch Sysmon config** sang Olaf Hartong's `sysmon-modular` (e10 default rộng hơn).
2. **Hoặc** restart Win10 VM hoàn toàn — force Sysmon driver reload từ scratch.
3. **Hoặc** uninstall + reinstall Sysmon với config mới.
4. **Trigger manual** bằng PowerShell PInvoke OpenProcess + CloseHandle (KHÔNG cần ProcDump, KHÔNG cần Antigravity) — benign red-team probe.
5. **Verify** e10 ship vào ES → R2 fire.
